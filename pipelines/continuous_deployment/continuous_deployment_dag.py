...
def get_branch_by_api_status() -> list[str] | str:
    ...
    try:
        ...
        if response.status_code == status.HTTP_200_OK: #200값 -> fastapi에 status으로 정의된... 이해하기 쉽다.. 숫자만으론 어려워~
    ...
    try:
        ...
        if response.status_code == status.HTTP_200_OK: #200값 -> fastapi에 status으로 정의된... 이해하기 쉽다.. 숫자만으론 어려워~
            ...
            return datetime.strptime(
                response.json().get("creation_time"), "%Y-%m-%dT%H:%M:%S.%fZ" # response.json() -> dictionary이므로 .get()으로, 그다음 형식을 datetime.strptime으로 파싱
            )
        else:
            print(
...
def get_latest_trained_model_creation_time() -> datetime | None:
    ...
    try:
        ...
        return bento_model.info.creation_time.replace(tzinfo=None) #timezone 정보 날리기...
    except Exception as e:
...
def decide_model_update():
    ...
    ): ## 그냥 elif으로 train..time > de...time 만 비교하면 되지 않나?
...
with DAG(
    ...
    deploy_new_model_task = BashOperator(
        ...
        trigger_rule="one_success", # dag에서 skip되는것 방지
    )

    # 배포를 건너뛸 경우 실행할 더미 태스크 -> EmptryOperator도 가능
